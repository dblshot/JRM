...
export default function EditTestForm({ test, onClose, onUpdate }) {
  const { lessons } = useAllLessons();
  const [selectedLessonId, setSelectedLessonId] = useState(test?.lessonId?._id || test?.lessonId || '');
  const [timeLimit, setTimeLimit] = useState(test?.timeLimit || 120);
  ...
  // Filter out lessons that already have tests (except current test's lesson)
  const availableLessons = lessons.filter(lesson => {
    if (lesson._id === selectedLessonId) return true; // Allow current lesson
    return true; // For editing, we'll allow changing to any lesson
  });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/tests/${test._id}`, {
        method: 'PUT',
  ...
  return (
    <div style={{ background: CARD_BG, borderRadius: 16, maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem', boxShadow: '0 4px 32px 0 rgba(0,0,0,0.18)' }}>
      <h2 style={{ color: ORANGE, fontWeight: 700, fontSize: 24, marginBottom: 4, textAlign: 'center' }}>Edit Test</h2>
      <p style={{ color: PLACEHOLDER_COLOR, fontSize: 15, marginBottom: 24, textAlign: 'center' }}>
        Update the test for "{test?.lessonId?.title || 'Unknown Lesson'}"
      </p>
      ...
      <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        {success && <Alert severity="success" sx={{ mt: 2 }}>Test updated successfully!</Alert>}
      </form>
    </div>
  );
}
